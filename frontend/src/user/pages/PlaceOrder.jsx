...
export default function PlaceOrder() {
  ...
  const handleCardChange = (e) => {
    ...
    // Format card number
    if (name === 'cardNumber') {
      const formatted = value.replace(/\s/g, '').match(/.{1,4}/g)?.join(' ') || value.replace(/\s/g, '');
        ...
        <div className="flex flex-col gap-8 lg:flex-row">
          ...
          <section>
            <div className="bg-white rounded-2xl border border-pink-200 shadow-[0_10px_30px_rgba(236,72,153,0.06)] p-6 mb-6">
              <h2 className="text-xl font-semibold text-slate-900 mb-6">Select Payment Method</h2>
              ...
              <form id="payment-form" onSubmit={handleSubmit}>
                ...
                <div className="space-y-4">
                  {/* Cash on Delivery */}
                  <label className={`payment-option flex items-start p-4 border-2 rounded-xl cursor-pointer hover:border-pink-400 transition-all bg-white ${
                    paymentMethod === 'Cash on Delivery' ? 'border-pink-500 bg-pink-50 shadow-[0_0_0_3px_rgba(236,72,153,0.1)]' : 'border-pink-200'
                  }`}>
                    <input 
                      type="radio" 
                  ...
                  {/* Credit/Debit Card */}
                  <label className={`payment-option flex items-start p-4 border-2 rounded-xl cursor-pointer hover:border-pink-400 transition-all bg-white ${
                    paymentMethod === 'Credit/Debit Card' ? 'border-pink-500 bg-pink-50 shadow-[0_0_0_3px_rgba(236,72,153,0.1)]' : 'border-pink-200'
                  }`}>
                    <input 
                      type="radio" 
                      name="payment_method_radio" 
            ...
            <div className="flex-1">
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <p className="text-sm text-slate-600 mt-1">Pay securely with your card</p>
                        </div>
                        <svg className="w-8 h-8 text-pink-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  {/* GCash */}
                  <label className={`payment-option flex items-start p-4 border-2 rounded-xl cursor-pointer hover:border-pink-400 transition-all bg-white ${
                    paymentMethod === 'GCash' ? 'border-pink-500 bg-pink-50 shadow-[0_0_0_3px_rgba(236,72,153,0.1)]' : 'border-pink-200'
                  }`}>
                    <input 
                      type="radio" 
                      name="payment_method_radio" 
          ...
          <div>
                  ...
                  {/* PayPal */}
                  <label className={`payment-option flex items-start p-4 border-2 rounded-xl cursor-pointer hover:border-pink-400 transition-all bg-white ${
                    paymentMethod === 'PayPal' ? 'border-pink-500 bg-pink-50 shadow-[0_0_0_3px_rgba(236,72,153,0.1)]' : 'border-pink-200'
                  }`}>
                    <input 
                      type="radio" 
                      name="payment_method_radio" 
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  <p className="text-sm text-yellow-800 mb-2">
                    <strong>Refund Policy:</strong> Cancellations are allowed up to 45 days before the event date or before production begins, whichever comes first. 
                    After this period, all orders are non-refundable.{' '}
                    <Link
                      to="/refund-policy"
              ...
              <div className="flex items-start gap-3">
                ...
                <label htmlFor="accept-terms" className="flex-1 text-sm text-slate-700 cursor-pointer">
                  ...
                  <p className="text-xs text-slate-500 mt-1">
                    By placing this order, you acknowledge that you have read and agree to our{' '}
                    <Link to="/refund-policy" className="text-pink-600 hover:text-pink-700 underline">
                      refund policy
                    </Link>
                    . Cancellations must be made at least 45 days before the event date or before production begins.
                  </p>
                </label>
              </div>
            </div>
          ...
          {/* Order Summary */}
            ...
            <button 
              ...
              disabled={!formData.firstName || !formData.lastName || !formData.email || 
                       !formData.address || !formData.city || !formData.zip || !acceptedTerms}
              className={`w-full rounded-md py-3 font-semibold text-white transition-all ${
                !formData.firstName || !formData.lastName || !formData.email || 
                !formData.address || !formData.city || !formData.zip || !acceptedTerms
                  ? 'bg-gray-400 cursor-not-allowed'
                  : 'bg-gradient-to-r from-pink-500 to-rose-500 hover:shadow-[0_0_30px_rgba(236,72,153,0.25)]'
              }`}
            >
              Complete Order
            </button>
