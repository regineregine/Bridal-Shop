...
export default function AdminCustomers() {
  ...
  const handleArchive = async (customerId) => {
    if (!window.confirm('Are you sure you want to archive this customer? They will be moved to archived customers.')) {
      return;
    }
  ...
  const handleUnarchive = async (customerId) => {
    if (!window.confirm('Are you sure you want to unarchive this customer? They will be moved back to active customers.')) {
      return;
    }
                  ...
                  {displayCustomers.length === 0 ? (
                    <tr>
                      <td colSpan="6" className="px-6 py-12 text-center text-slate-500">
                        {showArchived ? 'No archived customers found' : 'No active customers found'}
                      </td>
                    </tr>
                  ) : (
                    displayCustomers.map((customer) => (
                      <tr key={customer.id} className={`hover:bg-gray-50 ${showArchived ? 'opacity-75' : ''}`}>
                        ...
                        <td className="px-6 py-4">
                          <span className={`px-3 py-1 rounded-full text-xs font-medium ${
                            showArchived ? 'bg-gray-100 text-gray-700' : 'bg-blue-100 text-blue-700'
                          }`}>
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            {showArchived ? (
                              <button
                                ...
                                title={loadingArchive[customer.id] ? 'Unarchiving...' : 'Unarchive Customer'}
                              >
                              ...
                              <button
                                ...
                                title={loadingArchive[customer.id] ? 'Archiving...' : 'Archive Customer'}
                              >
                                {loadingArchive[customer.id] ? (
                                  <svg className="w-5 h-5 animate-spin" fill="none" viewBox="0 0 24 24">
                                    <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
